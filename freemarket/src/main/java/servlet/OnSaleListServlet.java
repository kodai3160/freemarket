...
@WebServlet("/onSaleList")
public class OnSaleListServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			error = "DB接続エラーの為、出品中の商品一覧表示は行えませんでした。";
			cmd = "login";
		} finally {
			//エラーの有無でフォワード先を呼び分ける
			if (error.equals("")) {
				//エラーが無い場合はonSaleList.jspにフォワード
				request.getRequestDispatcher("/view/onSaleList.jsp").forward(request, response);
			} else {
